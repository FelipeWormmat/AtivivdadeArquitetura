...
class Vehicle:
    ...
    def __init__ (self, model, catalogue_price, is_electric):
        ...
        self.id = CarRegistry.vehicle_id(12)
        self.license_plate = CarRegistry.vehicle_license(self.id)
        self.tax_percentage = TaxPercentage.tax_percentage(self)

class CarRegistry:
    def vehicle_id(length):
        return ''.join(random.choices(string.ascii_uppercase, k=length))
    def vehicle_license(id):
        first_section = id[:2]
...
class Application:
    def register_vehicle(self, is_electric: bool, catalogue_price: int, brand: string):
